...
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same `_SESSION_TOKEN` as REST (random per startup unless `HERMES_DASHBOARD_SESSION_TOKEN` is set), via query param (browsers can't set `Authorization` on WS upgrade).
